...
provider('ngStore', [function(){
  return {
    $get: ['NSModelFactory', function($NSMF) {
      return {
        model: function(name) {
        	return new $NSMF(name);
        }
      };
    }]
  };
}])
.factory('NSModelFactory', function(){
  ...
  Store.prototype = {
    ...
    findAll: function() {
      ...
      for (var i = 0; i < this.records.length; i++) {
        var id = this.records[i];
        var data = this.serializer.deserialize(this.localStorage().getItem(this._itemName(id)));
        if (data != null){
        	result.push(data);
        }
      }
